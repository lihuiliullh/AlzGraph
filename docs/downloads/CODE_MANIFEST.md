...
| KG-only MCQ baseline | `tasks/kg_baseline.py` | Deterministic, no-LLM baseline that answers MCQs from AlzKG evidence alone (lexicon NER + literature-weighted PPR); writes `data/alzkg/kg_baseline_results.json`. Measured on the fully corrected graph: T1 = 0.50 (n=20), T3 = 0.30 (n=10), T4 = 0.375 (n=16) accuracy vs. 0.25 random — T4 was exactly at chance (0.25) on the original graph, the clearest model-facing signal that the Sec. 2.5 fixes helped |
| AlzKG extraction-precision audit | `scripts/audit_kg_sample.py` | Draws a reproducible random sample of triplets (seed=42, and seed=43 for the final pooled draw) and resolves each one's grounding sentence via the recognized-entity index; the correctness judgment itself is manual. Three rounds are recorded in `data/alzkg/extraction_audit.json` (paper Sec. 2.5/Appendix B): original (4 correct / 8 partial / 18 incorrect), an interim post-lexicon-fix sample (6/12/12) later found to be on an incomplete corpus, and the final pooled n=60 result after also fixing a corpus-completeness bug (13 correct / 14 partial / 33 incorrect) |
...
- The released **AlzKG** (`data/alzkg/triplets.json`) is **mined from 7,150 real
  PMC open-access full-text papers** retrieved via NCBI E-utilities (320,299 candidate
  sentences in the final release; 361,201 under the original, less precise lexicon
  and incomplete corpus). Two independent bugs were found and fixed after the
  original release: (1) entity-disambiguation bugs in the lexicon (below), and
  (2) a data-completeness bug in which 13% of source papers (943 of 7,150) had
  silently fetched abstract-only content, almost certainly from transient PMC
  fetch failures during the original bulk fetch rather than a genuine absence of
  full text, since re-fetching the identical PMCIDs now succeeds. Both fixes are
  reflected in the released corpus and graph; `scripts/fetch_pmc_fulltext.py`,
  run fresh, reproduces this end to end.
...
- Sentence-level co-occurrence + trigger-phrase extraction is not error-free: a
  ...
  and separately found and fixed the corpus-completeness bug above. A final
  re-audit pooled across two independent 30-item draws (seeds 42 and 43, after
  finding a single draw too noisy to trust on its own) found 13 correct / 14
  partial / 33 incorrect out of 60 — a real but modest improvement (45% vs. 40%
  correct-or-partial) that still leaves further, now-dominant failure modes
  (gene-panel/list-co-occurrence artifacts, lab-technique-as-biomarker category
  errors, off-topic leakage, lost relation valence, unhandled negation) unfixed.
  See paper Sec. 2.5/Appendix B and the Limitations section.
