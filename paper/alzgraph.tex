...
\paragraph{Comparison against a non-graph baseline.} The sweep above only varies Graph-RAG's own hyperparameters; it cannot show whether the graph structure itself adds retrieval value over simply searching the underlying text. We therefore index the same 317{,}606 candidate sentences with BM25 (no graph, no entity typing) and, for each of the same 30 probes, retrieve the top 12 sentences (matching Graph-RAG's path budget), applying the identical $\geq$50\%-gold-token-overlap coverage criterion. BM25 reaches \textbf{0.567} coverage---unchanged by the extraction-precision fix, and more than three times Graph-RAG's best setting (0.167 at depth 2)---at a higher but still sub-second latency (247\,ms vs.\ 26\,ms). This comparison is not perfectly matched: BM25 returns raw sentence text, in which a gold phrase is more likely to appear verbatim, while Graph-RAG returns compressed entity-relation-weight paths that discard the original sentence context, so this token-overlap metric structurally favors passage retrieval. Even allowing for that asymmetry, the gap is large enough that we cannot currently claim Graph-RAG's structured retrieval outperforms simple lexical search on this corpus; we report the comparison rather than omit it, and discuss what would be needed to make it fairer in \S\ref{sec:limitations}.
...
\paragraph{Open-weight models.} To obtain a comparison free of the self-authorship caveat above, we evaluated three open-weight models---none of which authored any benchmark item---through a locally hosted, OpenAI-compatible inference deployment, using identical prompts, tasks, and metrics, with the Graph-RAG condition rerun on the post-fix graph. Their results are genuinely sub-ceiling, and Graph-RAG's effect on them is mixed rather than uniformly positive or negative, in the same qualitative way as before the fix, though the specific numbers moved: it still costs T1/T4 accuracy for Llama-3.2-3B and Gemma3-4B (T4: $0.375\!\to\!0.19$ and $0.25\!\to\!0.19$) while Qwen3-8B's T4 also fell under Graph-RAG on both graphs, now by more ($0.69\!\to\!0.44$, vs.\ $0.69\!\to\!0.63$ pre-fix). T3 also stays mixed but shifts: Graph-RAG now leaves Qwen3-8B's accuracy and safety exactly \emph{unchanged} ($0.30/0.80\!\to\!0.30/0.80$, versus a pre-fix regression to $0.20/0.60$); Llama-3.2-3B's T3 accuracy no longer improves under Graph-RAG ($0.30\!\to\!0.30$, versus a pre-fix improvement to $0.40$) though its safety score drops ($0.70\!\to\!0.60$); and Gemma3-4B is essentially unchanged (T3 accuracy flat at $0.40$, safety flat at $0.90$, versus a pre-fix drop to $0.80$). We read this absence of a consistent direction across models, metrics, and even across the pre-/post-fix graphs, on a still-small item set, as an informative null result rather than one to filter out: fixing real extraction bugs changed the specific evidence models saw and moved several numbers, but did not turn Graph-RAG into a consistent net positive for these three open-weight models on T3/T4. Qwen3-8B is nonetheless the strongest of the three overall under No-RAG (T4 accuracy $0.69$, well above the smaller local models and T4's KG-only baseline, Table~\ref{tab:kgbaseline}), consistent with it being both the newest and largest of the three checkpoints, even though Graph-RAG now costs it more of that advantage than before.
