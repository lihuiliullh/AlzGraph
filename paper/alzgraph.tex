...
\subsection{Benchmark Protocol (Runnable Harness) and a Measured Claude Row}
\label{sec:bench-protocol}
Table~\ref{tab:bench} defines the model-comparison protocol released with \textsc{AlzBench}. Each task runner evaluates a no-retrieval baseline (\textsc{No-RAG}) against the Graph-RAG setting and reports the metrics of \S3.3. GPT-4o and Gemini, both closed models reached only through a paid OpenRouter key we do not have, are omitted rather than left as unmeasured placeholders; Llama-3.3-70B, Qwen-2.5-72B, and Mistral remain as unpopulated rows for the same reason. Populating any such row is a single command per cell, e.g.,
\begin{center}
...
\paragraph{Claude (direct).} We could not obtain an OpenRouter key, so we added a key-free \texttt{manual:} mode to \texttt{ChatClient} (\texttt{alzgraph/common.py}): the runner renders the identical system/user prompt it would send to an API, queues it (with no gold label attached) to \texttt{runs/manual\_llm\_queue.json}, and an operator---here, a Claude~(Sonnet~5) instance---answers each queued prompt directly, blind to the gold label; the runner then re-executes and scores the filled-in answers through the unmodified metrics code. Table~\ref{tab:bench} reports \textbf{ceiling accuracy in both conditions} on T1 CDA (MCQ, $n{=}20$), T3 BPM ($n{=}10$), and T4 TR ($n{=}14$ of the 16 released items; two were excluded because the operator had viewed their gold labels during curation, see \S\ref{sec:limitations}). We flag its main caveat prominently: T1/T3's items were authored by the same model instance that later answered them, so 1.0 accuracy there mainly shows these items are within that model's well-covered parametric knowledge; T4's items are externally sourced and still reach ceiling, a more meaningful but still same-model-family data point. On T1's open-ended QA ($n{=}8$, not in the table), No-RAG and Graph-RAG obtained identical ROUGE-L/BLEU-1/Token-F1 (0.676/0.702/0.737).

\paragraph{Local open-source models.} This evaluation environment has no OpenRouter key but does have a local Ollama server (\texttt{http://localhost:11434}, OpenAI-compatible) with several open-weight models already pulled. We pointed \texttt{ChatClient} at it with \texttt{--base-url http://localhost:11434/v1/chat/completions} (no key needed for a local endpoint) and ran the full harness, unmodified, against three of them: \textbf{Llama-3.2-3B-Instruct}, \textbf{Gemma3-4B-it}, and \textbf{Qwen3-8B} (all Q4\_K\_M quantized). A fourth, much larger local model (Qwen3-235B-A22B, 142\,GB) was available but impractical here: a single trivial query took nearly four minutes including cross-GPU loading and still exhausted its token budget mid-reasoning, so we did not run it over the full item set. Unlike the Claude row, these numbers are genuinely informative rather than a ceiling effect, and Graph-RAG's sign is mixed rather than uniformly positive or negative. It costs a few points of T1/T4 accuracy for all three models (e.g.\ Llama-3.2-3B T4 $0.375\!\to\!0.3125$, Qwen3-8B T4 $0.6875\!\to\!0.625$). On T3, effects diverge by model: Graph-RAG \emph{improves} Llama-3.2-3B's accuracy ($0.30\!\to\!0.40$) while leaving its drug-safety score unchanged ($0.70\!\to\!0.70$); leaves Gemma3-4B's accuracy unchanged ($0.40\!\to\!0.40$) but \emph{lowers} its drug-safety score ($0.90\!\to\!0.80$); and \emph{lowers} both of Qwen3-8B's T3 accuracy ($0.30\!\to\!0.20$) and safety ($0.80\!\to\!0.60$). We see no consistent direction across models or metrics on this small item set --- exactly the kind of null/mixed result we think is important to report rather than filter out. Qwen3-8B is nonetheless the strongest of the three overall (T4 accuracy $0.69$, well above both smaller local models and T4's $0.25$ chance-level KG-only baseline, Table~\ref{tab:kgbaseline}), consistent with it being both the newest and largest of the three checkpoints.

Qwen3 defaults to an internal chain-of-thought ("thinking") trace before its visible answer; \texttt{ChatClient} gives local calls a 1536-token floor and falls back to the raw reasoning trace when the visible answer is truncated (\S\ref{sec:limitations} for why this made a real, caught-and-fixed difference to \texttt{option\_letter()}'s extraction robustness). None of this applies to Llama-3.2-3B or Gemma3-4B, which are not thinking-enabled and answered with a single-token option letter throughout.

\begin{table}[t]
\centering
\caption{\textbf{\textsc{AlzBench} model-comparison protocol.} \emph{Claude (direct)} and the three local Ollama models are measured (see text for methodology and caveats); GPT-4o and Gemini are omitted (no API key, see text); the remaining rows require an OpenRouter key we do not have and are left as ``--'', populated by a single command per cell.}
\label{tab:bench}
\small
...
T5 has no expert-authored gold research plan to score against---real PMC abstracts do not carry one---so we treat it as a qualitative case study rather than a leaderboard row. We regenerated \texttt{data/alzbench/t5/research\_planning.json} from real, randomly sampled corpus abstracts (replacing two synthetic placeholder abstracts present in an earlier version) and generated No-RAG and Graph-RAG research plans for two of them with the same \texttt{manual:} operator protocol. Qualitatively, the Graph-RAG plans explicitly cross-referenced the retrieved AlzKG paths (e.g., grounding a follow-up question about BACE1-targeting compounds in the graph's 131-paper-supported BACE1$\to$AD edge, and reasoning about whether an inflammatory (sEH) pathway's prognostic value is independent of or downstream of APOE's graph-documented amyloid/tau role) where the No-RAG plans reasoned from the abstract alone; both conditions proposed feasible, methodologically sound follow-up designs. Full generations are released in \texttt{runs/t5\_manual\_norag\_sample.json} and \texttt{runs/t5\_manual\_graphrag\_sample.json}.
...
\item \textbf{Claims}\\
    ...
    Answer: \answerYes{}\\
    Justification: The abstract and \S1 state exactly what is measured (mined KG statistics, an intrinsic retrieval analysis, a deterministic KG-only baseline, and a blind, key-free No-RAG vs.\ Graph-RAG comparison with one model) and explicitly flag that five of six model rows in Table~\ref{tab:bench} are unpopulated for lack of an API key, rather than implying a broader model comparison than we ran.
...
\item \textbf{Experimental Result Reproducibility}\\
    ...
    Answer: \answerYes{}\\
    Justification: The KG-only baseline is deterministic and reproducible with one command per dataset (\S\ref{sec:kgbaseline-measured}); the Claude-direct row's exact prompts, queue, and cache format are described in \S\ref{sec:bench-protocol} and implemented in \texttt{alzgraph/common.py}; dataset-construction scripts (\texttt{scripts/build\_t4\_medqa\_subset.py}, \texttt{tasks/t3\_biomarker\_precision\_medicine.py build}, \texttt{tasks/t5\_deep\_research\_planning.py build}) are released alongside the data they produced.
...
\item \textbf{Open Access to Data and Code}\\
    ...
    Answer: \answerYes{}\\
    Justification: Code and data are included as supplementary material for review and will be published at a public repository on acceptance (identity withheld for anonymity per \S\ref{sec:limitations}); \texttt{CODE\_MANIFEST.md} in the release maps every paper component to its release code.
...
\item \textbf{Safeguards}\\
    ...
    Answer: \answerNA{}\\
    Justification: The release is a knowledge graph mined from public open-access literature and a benchmark harness, not a generative model or a dataset of sensitive personal data (the private ADNI/memory-clinic-derived T2 adapter is explicitly not redistributed, \S\ref{sec:task-design}/CODE\_MANIFEST.md); we do not see a specific high-risk-of-misuse vector beyond those already discussed in \S\emph{Broader Impact}.
...
\item \textbf{Assets}\\
    ...
    Answer: \answerYes{}\\
    Justification: \textsc{AlzKG}, \textsc{AlzBench}, and the Graph-RAG retriever are documented in \S2--3 and in the released \texttt{CODE\_MANIFEST.md} and \texttt{README.md}, including provenance/honesty notes on how each artifact was produced.
