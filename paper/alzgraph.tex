...
Table~\ref{tab:bench} defines the model-comparison protocol released with \textsc{AlzBench}. Each task runner evaluates a no-retrieval baseline (\textsc{No-RAG}) against the Graph-RAG setting and reports the metrics of \S3.3. Five of the six rows are left unpopulated: those values depend on a third-party API key (OpenRouter) that this evaluation environment does not have, and we report only measurements we have actually run. Populating any such row is a single command per cell, e.g.,
\begin{center}
...
For the \emph{Claude (direct)} row we could not obtain an API key, so we instead added a key-free \texttt{manual:} mode to \texttt{ChatClient} (\texttt{alzgraph/common.py}): the runner renders the identical system/user prompt it would send to an API, queues it (with no gold label attached) to \texttt{runs/manual\_llm\_queue.json}, and an operator---here, a Claude~(Sonnet~5) instance---answers each queued prompt directly, blind to the gold label, exactly as it would if reached over the API; the runner then re-executes and scores the filled-in answers through the unmodified metrics code. Table~\ref{tab:bench} reports the result: \textbf{ceiling accuracy in both No-RAG and Graph-RAG} on T1 CDA (MCQ, $n{=}20$), T3 BPM ($n{=}10$), and T4 TR ($n{=}14$ of the 16 released items; two were excluded because the operator had viewed their gold labels during dataset curation, see \S\ref{sec:limitations}), with no measurable Graph-RAG gain. We report this ceiling effect rather than omit it, but flag its main caveat prominently: T1/T3's items were authored by the same model instance that later answered them, so 1.0 accuracy there mainly shows these items are within that model's well-covered parametric knowledge, not that Graph-RAG is redundant in general. T4's items are externally sourced (real MedQA-USMLE questions, not authored by the operator) and still reach ceiling, which is a more meaningful data point but still reflects one model family evaluating itself. On T1's open-ended QA ($n{=}8$, not shown in the table, which reports MCQ accuracy only), No-RAG and Graph-RAG obtained identical ROUGE-L/BLEU-1/Token-F1 (0.676/0.702/0.737) because the operator's answers were unaffected by the retrieved evidence for that item set; T4's Graph-RAG KG-evidence-coverage (0.018) is likewise near zero, indicating the retrieved paths rarely named the same treatment terms as the chosen option on this externally sourced task. Taken together with T4's chance-level KG-only baseline (Table~\ref{tab:kgbaseline}), the honest reading is that our current item pools do not yet discriminate Graph-RAG from a strong model's parametric knowledge; \S\ref{sec:limitations} discusses what would.

\begin{table}[t]
\centering
\caption{\textbf{\textsc{AlzBench} model-comparison protocol.} \emph{Claude (direct)} is measured (see text for the key-free evaluation method and its caveats); all other rows require an OpenRouter API key we do not have and are left as ``--'', populated by a single command per cell.}
\label{tab:bench}
\small
...
\paragraph{Item scale and self-authorship.} T1's MCQ/QA and T3's items are curated, hand-authored exam-style questions (20, 8, and 10 items respectively after this revision's expansion from 10/3/5); T4 draws 16 items from real MedQA-USMLE via a dementia-term filter; T5 currently offers 20 real abstracts but only 2 have been used for the qualitative case study. These are still small samples for a benchmark claim, and T1/T3 in particular were authored by the same model family used to evaluate them in Table~\ref{tab:bench}, which we believe explains their ceiling accuracy more than any special capability of Graph-RAG. A more discriminating benchmark would (i) grow each item pool by an order of magnitude, (ii) source items independently of the evaluated model (as T4 already does), and (iii) deliberately include harder, rarer, or adversarially constructed cases (atypical presentations, conflicting guideline edge cases, recently updated appropriate-use criteria) where a model's parametric knowledge is less likely to already be saturated.
...
\item \textbf{Experiments Compute Resources}\\
    ...
    Answer: \answerYes{}\\
    Justification: All reported experiments (KG mining, PPR retrieval, the KG-only baseline, and the Claude-direct evaluation) run on CPU in well under an hour on a single machine, with no GPU or distributed training involved; \S\ref{sec:retrieval-analysis} reports mean per-query retrieval latency (30--70\,ms).
...
\item \textbf{Declaration of LLM Usage}\\
    ...
    Answer: \answerYes{}\\
    Justification: An LLM (Claude, Sonnet~5) is a core, disclosed part of the methodology in \S\ref{sec:bench-protocol}: it both authored some T1/T3 benchmark items and, separately, served as the evaluated model for the Claude-direct row, blind to gold labels during the latter role. This dual role and its implications for the reported ceiling accuracy are discussed explicitly in \S\ref{sec:bench-protocol} and \S\ref{sec:limitations} rather than left implicit.
