...
Table~\ref{tab:bench} defines the model-comparison protocol released with \textsc{AlzBench}. Each task runner evaluates a no-retrieval baseline (\textsc{No-RAG}) against the Graph-RAG setting and reports the metrics of \S3.3, e.g.,
\begin{center}
\texttt{python tasks/t1\_clinical\_decision\_accuracy.py --dataset data/alzbench/t1/mcq.json}\\
\texttt{\ \ --base-url http://localhost:11434/v1/chat/completions --model llama3.2:latest --mode graph\_rag}
\end{center}
and the harness writes per-item predictions and aggregate scores to \texttt{runs/}. Any other model reachable through an OpenAI-compatible endpoint (OpenRouter-hosted or local) can be added the same way.
...
\begin{table}[t]
\centering
\caption{\textbf{\textsc{AlzBench} model-comparison protocol.} \emph{Claude (direct)} and the three local Ollama models are measured (see text for methodology and caveats). The harness accepts any OpenAI-compatible model (OpenRouter-hosted or local) via a single command per cell (\S\ref{sec:bench-protocol}), so this table can be extended with additional rows.}
\label{tab:bench}
\small
...
\item \textbf{Claims}\\
    ...
    Answer: \answerYes{}\\
    Justification: The abstract and \S1 state exactly what is measured (mined KG statistics, an intrinsic retrieval analysis, a deterministic KG-only baseline, and a blind, key-free No-RAG vs.\ Graph-RAG comparison run against four models: Claude direct and three local open-weight models), rather than implying a broader model comparison than we ran.
